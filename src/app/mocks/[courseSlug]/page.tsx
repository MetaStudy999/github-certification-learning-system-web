...
export default async function MockListPage({ params }: { params: Promise<{ courseSlug: string }> }) {
  ...
  try {
    ...
    return <main className="shell learningShell"><nav className="breadcrumbs"><Link href="/">Home</Link><span>/</span><span>Mock Exams</span></nav><section className="learningHeader"><div><p className="eyebrow">GH-900 · P6</p><h1>Mock Exams</h1><p>40문항 × 3회 · Mock 01/02는 60분, Final Mock은 55분 권장</p></div><Link className="sourceLink" href={`/readiness/${courseSlug}`}>Exam Readiness</Link></section><MockAttemptSummary courseSlug={courseSlug} /><section className="moduleGrid">{exams.map((exam) => <Link className="moduleCard" href={`/mocks/${courseSlug}/${exam.slug}`} key={exam.slug}><span className="code">{exam.code} · {exam.role}</span><h2>{exam.title}</h2><p>{exam.questions.length}문항 · 목표 {exam.targetPercent}%+ · 권장 {exam.recommendedMinutes}분</p></Link>)}</section></main>;
  } catch (error) {
